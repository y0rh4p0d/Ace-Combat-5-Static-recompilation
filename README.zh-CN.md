...
黑屏修好之后，画面仍有问题（天空被切成方块、水面出现大块条纹）。根因是**同一类错误的五种不同形态**：**拿"运行构建的地址"去比"写死的美版地址"**。这类错误**全部是静默的**——比较失败不报任何东西，只是那条渲染路径悄悄退回模拟实现。

前四种是**地址比较**：

| 位置 | 覆盖内容 | 症状 |
|---|---|---|
| 天空穹顶 `seg` 表 | 天空 | 用 32 个零长度线段建穹顶 → **天空全黑** |
| `rn_2d.c` 前端区间 | 菜单 / UI | 前端绘制被当成 3D 世界绘制 |
| `rn_screen.c` 13 个区间 | 天空通道、太阳、云、自阴影、**整个特效系统** | 全部退回模拟路径 |
| `rn_sun.c` 3 个常量 | 太阳光晕、镜头鬼影、遮挡链 | 同上 |

第五种是**地址读取**，也是最后一处，症状最隐蔽：

- 渲染器要把游戏的 **VU1 微程序**指令复制给模拟的 VU1，而它从**美版 `.vutext` 段地址**读
- 日版把整个 `.vutext` 段移了 **+0x340**，所以在汉化版上读到的是**无关字节**
- 于是解析找不到上传指令 → **`0 pairs`** → `prog_matches()` **拒绝所有上传** → **原生 VU1 几何完全没运行**
- 结果：3D 几何全部由模拟路径绘制，**原生渲染器的接入点收到的工作量始终是零**——这就是为什么前面四次修"合成层"看不到效果

这五处都改为**内容校验**而非假设偏移：微程序以 `0x60` 操作码开头，天空的圆形数表以 `0.0, 1.0, sin(π/16), cos(π/16)` 开头，两者在正确地址上成立、在错误地址上不成立。

### 3c. 反向地址映射

上面这些都依赖一个能力：**把"运行构建的地址"翻译回"参照构建的地址"**。这原本**完全不存在**（运行时只有少数几个按名字硬编码的正向映射）。

现在把构建工具算好的**整份地址映射表编译进运行时**（`tools/gen_revmap.py` → `rn_revmap_data.c`，约 25000 条区间，覆盖 `.text` 的 97.7%）。三个要点：

- **它是"每个构建各自的数据"**，不能共用：把日版的表编译进美版构建，会让美版拿日版偏移量翻译自己的地址，**产生静默的错误翻译**——比不翻译更糟。所以它生成到各区域的输出目录，缺失时编译空表（查询返回输入本身，对没移动的构建正好正确）。
- **正向映射并非一一对应**：有 204 个区间里两个不同偏移的源区间落在同一段目标字节上。逆向上这些区间**无解**，所以被剔除，查询未命中时调用方保持原值——**猜一个答案正是这东西最不能犯的错**。
- 偏移量的符号极易搞反（`source - target` 写成 `target - source`），反了**每个翻译都错两倍偏移，而表本身看起来完全正常**。只能靠核对已知对应关系来发现。
...
汉化版目前有**两个未解决的问题**，完整的排查记录（现象、已验证的事实、试过什么、
下一步方向）在 [**`docs/known-issues.md`**](docs/known-issues.md)：

1. **无线电语音语言设置无效** —— 新游戏里能把语言设成英语并确认，但任务中仍然是
   日语语音。PCSX2 跑同一张盘选英语**能出英语语音**，所以这是我们的问题，不是汉化
   补丁的限制。
2. **本机无法进入独占全屏** —— 驱动拒绝一切显示模式切换（连普通 `1920x1080 @ 60 Hz`
   都拒绝），而独占全屏的定义就是切换显示模式。无边框全屏**可以正常使用**；独占的
   代码路径已经写好，在驱动允许的机器上会自动生效。

该文件同时记录了这几个问题排查过程中**犯过的错误**（读错地址、读错偏移、读错时机），
避免下次重复。
